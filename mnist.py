...
def load_dataset():
    ...
    X_train = np.array(map(lambda i: i['depth'], TrainData))
    y_train = np.array(map(lambda i: i['lbl'], TrainData))
    X_test = np.array(map(lambda i: i['depth'], TestData))
    y_test = np.array(map(lambda i: i['lbl'], TestData))
    X_val = np.array(map(lambda i: i['depth'], ValidationData))
    y_val = np.array(map(lambda i: i['lbl'], ValidationData))
...
def build_mlp(input_var=None):
    ...
    l_in = lasagne.layers.InputLayer(shape=(None, 1, 240, 320),
                                     input_var=input_var)
